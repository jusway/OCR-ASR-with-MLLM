...
DRAFT_SELECTED_MODEL = "deepseek-pro-nothink"
draft_system_prompt = """\
...
# ── 步骤 ②：精准原文 ───
# ============================================================
# deepseek-flash-nothink智商不够，连你的命令都听不懂。
# deepseek-flash-max 推理来推理去，还是笨，脑袋糊糊的，推理了一堆，还是不精确，往多了走
# deepseek-pro-nothink有概率遗漏。
# deepseek-pro-high有概率不行，要么多要么少，不精准,是不是脑子聪明了但是思考不够。
REFINE_SELECTED_MODEL = "deepseek-pro-max"
...
EXTRACT_SELECTED_MODEL = "deepseek-pro-nothink"
extract_system_prompt = """你是一位严谨的文献整理员。"""
extract_user_prompt_template = """\
【资料】
{precision_text}
## 背景
【资料】是《摩诃止观辅行传弘决辑注》的节选，一般包含四部分：
摩诃止观正文（一般是被**包裹）、摩诃止观科判（一般在摩诃止观正文前）、摩诃止观解释（一般在摩诃止观正文后）、注脚（一般是罗列在摩诃止观解释后面）。
## 任务
忽略资料中的科判、解释、注脚，只提取出**摩诃止观正文**的句子，按顺序直接拼接输出，不要使用换行符进行拼接。
直接输出结果，不说任何多余的话。
"""
# ============================================================
# ── 步骤 ③：最终校对稿 ───
...
# 任务文件夹（脚本自动找 .mp3）
folder_path = Path(r"../摩诃止观-定智法师 2017/07正说分/2018年7月13日摩诃止观07正说分第四摄法001")
# 元数据（写入校对稿头部）
year = "2018"
author = "定智法师"
# ── 执行 ──────────────────
...
if __name__ == "__main__":
    ...
    pipeline.run(
        ...
        extract_model_key=EXTRACT_SELECTED_MODEL,
