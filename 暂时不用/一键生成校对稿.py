...
from core.proofread_pipeline import run
...
原文参考是这位师父本次讲解的《摩诃止观辅行传弘决辑注》的大致原文范围，是我复制给你看的。
原文参考是docx复制到md文件中所得，包括四部分，摩诃止观正文（一般被**包裹住了）、摩诃止观科判（一般跟在正文前面）、\
摩诃止观解释（一般跟在正文后面）、注脚（一般在一起罗列）
## 任务
审核录音稿件是不是存在一部分师父讲解的**摩诃止观正文**在原文参考中没有收录。（比如讲解已经讲到后面的**摩诃止观正文**，但是后面的**摩诃止观正文**我复制的时候没有选中）
（稿件开头如果存在“前情提要”的**摩诃止观正文**，那个属于上节课，不在本节课考虑范围）
注意，只专注于**摩诃止观正文**我是不是给你复制全了。
如果发现录音稿件多讲了一部分**摩诃止观正文**，请你输出多讲了哪些**摩诃止观正文**。
如果确认没有多讲，那就输出“【没有多讲】”。
"""

# 录音稿件+大致原文=得到校对稿
text_system_prompt = """\
你是一位编辑。你擅长校对录音稿件，组织成书面语的校对稿，而且不会丢失任何的信息。这一点一直是你的骄傲。
"""
...
摩诃止观解释（一般跟在正文后面）、注脚（一般在一起罗列）
## 任务
参考优秀案例，校对录音稿件，直接输出书面语的校对稿的正文内容。
"""
...
摩诃止观解释（一般跟在正文后面）、注脚（一般在一起罗列）
## 任务
找到【讲稿】的内容对应在讲解【复制的节选】中**摩诃止观正文**的哪些句子（按顺序）。
