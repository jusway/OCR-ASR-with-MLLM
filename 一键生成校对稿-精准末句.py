...
## 任务
讲稿末尾必定是在讲解模糊原文参考的中间某一片段，找到这个片段，然后原封不动地输出这个片段的下一个科判即可。（比如 “△二明大九，初释名略示观法”）（我需要下一个科判复制粘贴搜索定位）
...
## 背景
ASR语音识别稿件是一位师父在讲解《摩诃止观辅行传弘决辑注》的记录，但是包含错别字、同音字和啰嗦语气词等。
原文参考是本节课的《摩诃止观辅行传弘决辑注》的节选，有四个结构：科判、摩诃止观原文（一般被**包裹）、解释、注脚。

## 任务
请对上述【ASR语音识别稿件】进行书面语校对。
直接输出校对后的正文，按语义自然分段，不要任何客套话、解释或开头结尾的过渡语。
虽然语音稿没有这么做，但是你要在每一大段对摩诃止观原文的讲解前面，开一个自然段只写用**强调的原文，类似把摩诃止观原文作为小节标题一般。

## 核心纪律
- 逐字逐句转化：必须保留讲演者的**所有**信息细节、举例和逻辑推演、乃至开玩笑的话都要保留，绝对禁止任何形式的浓缩、总结、概括或删减！
- 书面语校对就是仅修正错别字、同音字错误，去除口语化的冗余语气词（如“啊”、“那个”、“就是说”等），理顺语序。
"""
...
# 任务文件夹（脚本自动找 .mp3）
folder_path = Path(r"摩诃止观-定智法师 2017/09正说分/2018年11月8日摩诃止观09正说分第六方便005")
