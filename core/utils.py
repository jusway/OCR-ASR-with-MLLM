...
class AudioCompressor:
    ...
    def compress(audio_path: str) -> tuple[str, bool]:
        ...
        print(f"{Color.DARK_PURPLE}[AudioCompressor] 正在压缩音频 (原大小: {file_size / 1024 / 1024:.2f} MB)...{Color.RESET}")
        from pydub import AudioSegment
        ...
        print(f"{Color.GREEN}[AudioCompressor] 压缩完成，新文件大小: {os.path.getsize(temp_compressed_path) / 1024 / 1024:.2f} MB{Color.RESET}")
...
class OSSAudioUploader:
    ...
    def upload(self, local_path: str, filename: str = None) -> tuple[str, str]:
        ...
        if self.bucket.object_exists(object_key):
            print(f"{Color.GREEN}[OSS] 文件已存在，直接复用: {object_key}{Color.RESET}")
        else:
            self.bucket.put_object_from_file(object_key, local_path)
            print(f"{Color.GREEN}[OSS] 已上传: {object_key}{Color.RESET}")
    ...
    def delete(self, object_key: str):
        ...
        print(f"{Color.DARK_PURPLE}[OSS] 已删除: {object_key}{Color.RESET}")
    ...
    def cleanup_all(self):
        ...
        if keys:
            ...
            print(f"{Color.DARK_PURPLE}[OSS] 已清理 {len(keys)} 个临时文件{Color.RESET}")
